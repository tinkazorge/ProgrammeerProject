package org.andengine.entity.sprite.batch.vbo.andengine.util.modifier;
...
public class LoopModifier<T> extends BaseModifier<T> implements IModifierListener<T> {
	...
	public LoopModifier(final IModifier<T> pModifier) {
		this(pModifier, org.andengine.util.modifier.LoopModifier.LOOP_CONTINUOUS);
	}
	...
	public LoopModifier(final IModifier<T> pModifier, final int pLoopCount, final ILoopModifierListener<T> pLoopModifierListener, final IModifierListener<T> pModifierListener) {
		...
		this.mLoop = 0;
		this.mDuration = (pLoopCount == org.andengine.util.modifier.LoopModifier.LOOP_CONTINUOUS) ? Float.POSITIVE_INFINITY : pModifier.getDuration() * pLoopCount; // TODO Check if POSITIVE_INFINITY works correct with i.e. SequenceModifier

		this.mModifier.addModifierListener(this);
	}

	protected LoopModifier(final org.andengine.util.modifier.LoopModifier<T> pLoopModifier) throws DeepCopyNotSupportedException {
		this(pLoopModifier.mModifier.deepCopy(), pLoopModifier.mLoopCount);
	}

	@Override
	public org.andengine.util.modifier.LoopModifier<T> deepCopy() throws DeepCopyNotSupportedException {
		return new org.andengine.util.modifier.LoopModifier<T>(this);
	}
	...
	public void onModifierFinished(final IModifier<T> pModifier, final T pItem) {
		...
		if (this.mLoopCount == org.andengine.util.modifier.LoopModifier.LOOP_CONTINUOUS) {
			this.mSecondsElapsed = 0;
			this.mModifier.reset();
		} else {
			this.mLoop++;
	...
	public interface ILoopModifierListener<T> {
		public void onLoopStarted(final org.andengine.util.modifier.LoopModifier<T> pLoopModifier, final int pLoop, final int pLoopCount);
		public void onLoopFinished(final org.andengine.util.modifier.LoopModifier<T> pLoopModifier, final int pLoop, final int pLoopCount);
	}
}
