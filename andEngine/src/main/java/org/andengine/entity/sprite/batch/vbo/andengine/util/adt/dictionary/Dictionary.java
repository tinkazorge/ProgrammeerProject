package org.andengine.entity.sprite.batch.vbo.andengine.util.adt.dictionary;
...
public class Dictionary {
	...
	private static final int OFFSET_CHARACTER_COUNT = org.andengine.util.adt.dictionary.Dictionary.OFFSET_VERSION + org.andengine.util.adt.dictionary.Dictionary.SIZE_VERSION;
	private static final int SIZE_CHARACTER_COUNT = Integer.SIZE;

	private static final int OFFSET_CHARACTER_BITLENGTH = org.andengine.util.adt.dictionary.Dictionary.OFFSET_CHARACTER_COUNT + org.andengine.util.adt.dictionary.Dictionary.SIZE_CHARACTER_COUNT;
	private static final int SIZE_CHARACTER_BITLENGTH = Byte.SIZE;

	private static final int OFFSET_INDEXENTRY_COUNT = org.andengine.util.adt.dictionary.Dictionary.OFFSET_CHARACTER_BITLENGTH + org.andengine.util.adt.dictionary.Dictionary.SIZE_CHARACTER_BITLENGTH;
	private static final int SIZE_INDEXENTRY_COUNT = Integer.SIZE;

	private static final int OFFSET_INDEXENTRY_STARTINDEX_BITLENGTH = org.andengine.util.adt.dictionary.Dictionary.OFFSET_INDEXENTRY_COUNT + org.andengine.util.adt.dictionary.Dictionary.SIZE_INDEXENTRY_COUNT;
	private static final int SIZE_INDEXENTRY_STARTINDEX_BITLENGTH = Byte.SIZE;

	private static final int OFFSET_INDEXENTRY_LENGTH_BITLENGTH = org.andengine.util.adt.dictionary.Dictionary.OFFSET_INDEXENTRY_STARTINDEX_BITLENGTH + org.andengine.util.adt.dictionary.Dictionary.SIZE_INDEXENTRY_STARTINDEX_BITLENGTH;
	private static final int SIZE_INDEXENTRY_LENGTH_BITLENGTH = Byte.SIZE;

	private static final int OFFSET_CHARACTERS = org.andengine.util.adt.dictionary.Dictionary.OFFSET_INDEXENTRY_LENGTH_BITLENGTH + org.andengine.util.adt.dictionary.Dictionary.SIZE_INDEXENTRY_LENGTH_BITLENGTH;
	...
	public Dictionary(final IBitVector pBitVector) {
		...
		this.mVersion = this.mBitVector.getShort(org.andengine.util.adt.dictionary.Dictionary.OFFSET_VERSION);
		if (this.mVersion != org.andengine.util.adt.dictionary.Dictionary.VERSION) {
			throw new IllegalArgumentException("Illegal version: " + this.mVersion);
		}

		this.mCharacterCount = this.mBitVector.getInt(org.andengine.util.adt.dictionary.Dictionary.OFFSET_CHARACTER_COUNT);
		this.mCharacterBitLength = this.mBitVector.getByte(org.andengine.util.adt.dictionary.Dictionary.OFFSET_CHARACTER_BITLENGTH);
		this.mCharacterTable = new char[this.mCharacterCount];
		for (int i = 0; i < this.mCharacterCount; i++) {
			this.mCharacterTable[i] = (char) this.mBitVector.getShort(org.andengine.util.adt.dictionary.Dictionary.OFFSET_CHARACTERS + (i * Character.SIZE));
		}

		this.mEntryCount = this.mBitVector.getInt(org.andengine.util.adt.dictionary.Dictionary.OFFSET_INDEXENTRY_COUNT);
		this.mEntryStartIndexBitLength = this.mBitVector.getByte(org.andengine.util.adt.dictionary.Dictionary.OFFSET_INDEXENTRY_STARTINDEX_BITLENGTH);
		this.mEntryLengthBitLength = this.mBitVector.getByte(org.andengine.util.adt.dictionary.Dictionary.OFFSET_INDEXENTRY_LENGTH_BITLENGTH);

		this.mEntriesOffset = org.andengine.util.adt.dictionary.Dictionary.OFFSET_CHARACTERS + (this.mCharacterCount * Character.SIZE);
	...
	public static org.andengine.util.adt.dictionary.Dictionary load(final InputStream pInputStream) throws IOException {
		return new org.andengine.util.adt.dictionary.Dictionary(ByteBackedBitVector.load(new DataInputStream(pInputStream)));
	}
	...
	public static class Factory {
		...
		public static org.andengine.util.adt.dictionary.Dictionary create(final File pFile) throws IOException {
			return org.andengine.util.adt.dictionary.Dictionary.Factory.create(FileUtils.readLines(pFile));
		}

		public static org.andengine.util.adt.dictionary.Dictionary create(final InputStream pInputStream) throws IOException {
			return org.andengine.util.adt.dictionary.Dictionary.Factory.create(StreamUtils.readLines(pInputStream));
		}

		public static org.andengine.util.adt.dictionary.Dictionary create(final Reader pReader) throws IOException {
			return org.andengine.util.adt.dictionary.Dictionary.Factory.create(StreamUtils.readLines(pReader));
		}

		public static org.andengine.util.adt.dictionary.Dictionary create(final String ... pStrings) {
			Arrays.sort(pStrings, org.andengine.util.adt.dictionary.Dictionary.CASEINSENSITIVE_REVERSE_LEXICOGRAPHICAL_COMPARATOR);

			final org.andengine.util.adt.dictionary.Dictionary.Factory.Entries entryDictionary = org.andengine.util.adt.dictionary.Dictionary.Factory.createDictionary(pStrings);
			...
			final int bitsRequired = org.andengine.util.adt.dictionary.Dictionary.Factory.calculateBitsRequired(characterCountTotal, characterCount, characterBitLength, entryCount, entryStartIndexBitLength, entryLengthBitLength);

			final IBitVector bitVector = new ByteBackedBitVector(bitsRequired);

			/* Write meta data. */
			bitVector.setShort(org.andengine.util.adt.dictionary.Dictionary.OFFSET_VERSION, org.andengine.util.adt.dictionary.Dictionary.VERSION);
			bitVector.setInt(org.andengine.util.adt.dictionary.Dictionary.OFFSET_CHARACTER_COUNT, characterCount);
			bitVector.setByte(org.andengine.util.adt.dictionary.Dictionary.OFFSET_CHARACTER_BITLENGTH, (byte) characterBitLength);
			bitVector.setInt(org.andengine.util.adt.dictionary.Dictionary.OFFSET_INDEXENTRY_COUNT, entryCount);
			bitVector.setByte(org.andengine.util.adt.dictionary.Dictionary.OFFSET_INDEXENTRY_STARTINDEX_BITLENGTH, (byte) entryStartIndexBitLength);
			bitVector.setByte(org.andengine.util.adt.dictionary.Dictionary.OFFSET_INDEXENTRY_LENGTH_BITLENGTH, (byte) entryLengthBitLength);

			/* Character table. */
			for (int i = 0; i < characterCount; i++) {
				final char character = entryDictionary.mCharacters[i];
				final short characterBits = (short) character;
				bitVector.setShort(org.andengine.util.adt.dictionary.Dictionary.OFFSET_CHARACTERS + (i * Character.SIZE), characterBits);
			}

			/* Index entries. */
			final int offsetEntries = org.andengine.util.adt.dictionary.Dictionary.OFFSET_CHARACTERS + (characterCount * Character.SIZE);
			...
			return new org.andengine.util.adt.dictionary.Dictionary(bitVector);
		}
		...
		private static int calculateBitsRequired(final int pCharacterCountTotal, final int pCharacterCount, final int pCharacterBitLength, final int pEntryCount, final int pEntryStartIndexBitLength, final int pEntryLengthBitLength) {
			return org.andengine.util.adt.dictionary.Dictionary.SIZE_VERSION
					+ org.andengine.util.adt.dictionary.Dictionary.SIZE_CHARACTER_COUNT
					+ org.andengine.util.adt.dictionary.Dictionary.SIZE_CHARACTER_BITLENGTH
					+ org.andengine.util.adt.dictionary.Dictionary.SIZE_INDEXENTRY_COUNT
					+ org.andengine.util.adt.dictionary.Dictionary.SIZE_INDEXENTRY_STARTINDEX_BITLENGTH
					+ org.andengine.util.adt.dictionary.Dictionary.SIZE_INDEXENTRY_LENGTH_BITLENGTH
		...
		private static Entries createDictionary(final String ... pStrings) {
			...
			for (final String string : pStrings) {
				...
				if (existingStringStartIndex >= 0) {
					...
					final int prefixLength = org.andengine.util.adt.dictionary.Dictionary.Factory.getPrefixLength(stringBuilder, string);
					if (prefixLength == -1) {
						/* Append new string. */
			...
			return new org.andengine.util.adt.dictionary.Dictionary.Factory.Entries(entries, stringBuilder.toString());
		}
