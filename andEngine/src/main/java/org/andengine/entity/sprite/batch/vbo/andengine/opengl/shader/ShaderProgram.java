...
public class ShaderProgram {
	...
	private static final byte[] NAME_CONTAINER = new byte[org.andengine.opengl.shader.ShaderProgram.NAME_CONTAINER_SIZE];
...
//		pGLState.useProgram(0); // TODO Does this have an positive/negative impact on performance?
	...
	protected void compile(final GLState pGLState) throws ShaderProgramException {
		...
		final int vertexShaderID = org.andengine.opengl.shader.ShaderProgram.compileShader(vertexShaderSource, GLES20.GL_VERTEX_SHADER);

		final String fragmentShaderSource = this.mFragmentShaderSource.getShaderSource(pGLState);
		final int fragmentShaderID = org.andengine.opengl.shader.ShaderProgram.compileShader(fragmentShaderSource, GLES20.GL_FRAGMENT_SHADER);
	...
	protected void link(final GLState pGLState) throws ShaderProgramLinkException {
		...
		GLES20.glGetProgramiv(this.mProgramID, GLES20.GL_LINK_STATUS, org.andengine.opengl.shader.ShaderProgram.HARDWAREID_CONTAINER, 0);
		if (org.andengine.opengl.shader.ShaderProgram.HARDWAREID_CONTAINER[0] == 0) {
	...
	private static int compileShader(final String pSource, final int pType) throws ShaderProgramException {
		...
		GLES20.glGetShaderiv(shaderID, GLES20.GL_COMPILE_STATUS, org.andengine.opengl.shader.ShaderProgram.HARDWAREID_CONTAINER, 0);
		if (org.andengine.opengl.shader.ShaderProgram.HARDWAREID_CONTAINER[0] == 0) {
	...
	private void initUniformLocations() throws ShaderProgramLinkException {
		...
		org.andengine.opengl.shader.ShaderProgram.PARAMETERS_CONTAINER[0] = 0;
		GLES20.glGetProgramiv(this.mProgramID, GLES20.GL_ACTIVE_UNIFORMS, org.andengine.opengl.shader.ShaderProgram.PARAMETERS_CONTAINER, 0);
		final int numUniforms = org.andengine.opengl.shader.ShaderProgram.PARAMETERS_CONTAINER[0];

		for (int i = 0; i < numUniforms; i++) {
			GLES20.glGetActiveUniform(this.mProgramID, i, org.andengine.opengl.shader.ShaderProgram.NAME_CONTAINER_SIZE, org.andengine.opengl.shader.ShaderProgram.LENGTH_CONTAINER, 0, org.andengine.opengl.shader.ShaderProgram.SIZE_CONTAINER, 0, org.andengine.opengl.shader.ShaderProgram.TYPE_CONTAINER, 0, org.andengine.opengl.shader.ShaderProgram.NAME_CONTAINER, 0);

			int length = org.andengine.opengl.shader.ShaderProgram.LENGTH_CONTAINER[0];

			/* Some drivers do not report the actual length here, but zero. Then the name is '\0' terminated. */
			if (length == 0) {
				while ((length < org.andengine.opengl.shader.ShaderProgram.NAME_CONTAINER_SIZE) && (org.andengine.opengl.shader.ShaderProgram.NAME_CONTAINER[length] != '\0')) {
					length++;
				}
			}

			String name = new String(org.andengine.opengl.shader.ShaderProgram.NAME_CONTAINER, 0, length);
			...
			if (location == ShaderProgramConstants.LOCATION_INVALID) {
				...
				name = new String(org.andengine.opengl.shader.ShaderProgram.NAME_CONTAINER, 0, length);
	...
	private void initAttributeLocations() {
		...
		org.andengine.opengl.shader.ShaderProgram.PARAMETERS_CONTAINER[0] = 0;
		GLES20.glGetProgramiv(this.mProgramID, GLES20.GL_ACTIVE_ATTRIBUTES, org.andengine.opengl.shader.ShaderProgram.PARAMETERS_CONTAINER, 0);
		final int numAttributes = org.andengine.opengl.shader.ShaderProgram.PARAMETERS_CONTAINER[0];

		for (int i = 0; i < numAttributes; i++) {
			GLES20.glGetActiveAttrib(this.mProgramID, i, org.andengine.opengl.shader.ShaderProgram.NAME_CONTAINER_SIZE, org.andengine.opengl.shader.ShaderProgram.LENGTH_CONTAINER, 0, org.andengine.opengl.shader.ShaderProgram.SIZE_CONTAINER, 0, org.andengine.opengl.shader.ShaderProgram.TYPE_CONTAINER, 0, org.andengine.opengl.shader.ShaderProgram.NAME_CONTAINER, 0);
