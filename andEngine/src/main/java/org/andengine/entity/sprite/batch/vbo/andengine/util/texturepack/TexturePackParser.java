...
//	private static final String TAG_TEXTURE_ATTRIBUTE_HEIGHT = "height";
	...
	private org.andengine.util.texturepack.TexturePack mTexturePack;
	...
	public org.andengine.util.texturepack.TexturePack getTexturePack() {
	...
	public void startElement(final String pUri, final String pLocalName, final String pQualifiedName, final Attributes pAttributes) throws SAXException {
		if (pLocalName.equals(org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE)) {
			this.mVersion = SAXUtils.getIntAttributeOrThrow(pAttributes, org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_VERSION);
		...
		} else if (pLocalName.equals(org.andengine.util.texturepack.TexturePackParser.TAG_TEXTUREREGION)) {
			final int id = SAXUtils.getIntAttributeOrThrow(pAttributes, org.andengine.util.texturepack.TexturePackParser.TAG_TEXTUREREGION_ATTRIBUTE_ID);
			final int x = SAXUtils.getIntAttributeOrThrow(pAttributes, org.andengine.util.texturepack.TexturePackParser.TAG_TEXTUREREGION_ATTRIBUTE_X);
			final int y = SAXUtils.getIntAttributeOrThrow(pAttributes, org.andengine.util.texturepack.TexturePackParser.TAG_TEXTUREREGION_ATTRIBUTE_Y);
			final int width = SAXUtils.getIntAttributeOrThrow(pAttributes, org.andengine.util.texturepack.TexturePackParser.TAG_TEXTUREREGION_ATTRIBUTE_WIDTH);
			final int height = SAXUtils.getIntAttributeOrThrow(pAttributes, org.andengine.util.texturepack.TexturePackParser.TAG_TEXTUREREGION_ATTRIBUTE_HEIGHT);
			...
			final boolean trimmed = SAXUtils.getBooleanAttributeOrThrow(pAttributes, org.andengine.util.texturepack.TexturePackParser.TAG_TEXTUREREGION_ATTRIBUTE_TRIMMED);
			final boolean rotated = SAXUtils.getBooleanAttributeOrThrow(pAttributes, org.andengine.util.texturepack.TexturePackParser.TAG_TEXTUREREGION_ATTRIBUTE_ROTATED);
	...
	private ITexture parseTexture(final Attributes pAttributes) throws TexturePackParseException {
		final String file = SAXUtils.getAttributeOrThrow(pAttributes, org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_FILE);

		if (this.mTextureManager.hasMappedTexture(file)) {
			return this.mTextureManager.getMappedTexture(file);
		}

		final String type = SAXUtils.getAttributeOrThrow(pAttributes, org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_TYPE);
		final PixelFormat pixelFormat = org.andengine.util.texturepack.TexturePackParser.parsePixelFormat(pAttributes);

		final TextureOptions textureOptions = this.parseTextureOptions(pAttributes);

		final ITexture texture;
		if (type.equals(org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_TYPE_VALUE_BITMAP)) {
			try {
				texture = new BitmapTexture(this.mTextureManager, new IInputStreamOpener() {
					@Override
					public InputStream open() throws IOException {
						return org.andengine.util.texturepack.TexturePackParser.this.onGetInputStream(file);
					}
				}, BitmapTextureFormat.fromPixelFormat(pixelFormat), textureOptions);
			} catch (final IOException e) {
				throw new TexturePackParseException(e);
			}
		} else if (type.equals(org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_TYPE_VALUE_PVR)) {
			try {
		...
		} else if (type.equals(org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_TYPE_VALUE_PVRGZ)) {
			try {
		...
		} else if (type.equals(org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_TYPE_VALUE_PVRCCZ)) {
			try {
		...
		} else if (type.equals(org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_TYPE_VALUE_ETC1)) {
			try {
	...
	private static PixelFormat parsePixelFormat(final Attributes pAttributes) {
		return PixelFormat.valueOf(SAXUtils.getAttributeOrThrow(pAttributes, org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_PIXELFORMAT));
	}

	private TextureOptions parseTextureOptions(final Attributes pAttributes) {
		final int minFilter = org.andengine.util.texturepack.TexturePackParser.parseMinFilter(pAttributes);
		final int magFilter = org.andengine.util.texturepack.TexturePackParser.parseMagFilter(pAttributes);
	...
	private static int parseMinFilter(final Attributes pAttributes) {
		final String minFilter = SAXUtils.getAttributeOrThrow(pAttributes, org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_MINFILTER);
		if (minFilter.equals(org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_MINFILTER_VALUE_NEAREST)) {
			return GL10.GL_NEAREST;
		} else if (minFilter.equals(org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_MINFILTER_VALUE_LINEAR)) {
			return GL10.GL_LINEAR;
		} else if (minFilter.equals(org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_MINFILTER_VALUE_LINEAR_MIPMAP_LINEAR)) {
			return GL10.GL_LINEAR_MIPMAP_LINEAR;
		} else if (minFilter.equals(org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_MINFILTER_VALUE_LINEAR_MIPMAP_NEAREST)) {
			return GL10.GL_LINEAR_MIPMAP_NEAREST;
		} else if (minFilter.equals(org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_MINFILTER_VALUE_NEAREST_MIPMAP_LINEAR)) {
			return GL10.GL_NEAREST_MIPMAP_LINEAR;
		} else if (minFilter.equals(org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_MINFILTER_VALUE_NEAREST_MIPMAP_NEAREST)) {
			return GL10.GL_NEAREST_MIPMAP_NEAREST;
		} else {
			throw new IllegalArgumentException("Unexpected " + org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_MINFILTER + " attribute: '" + minFilter + "'.");
		}
	}

	private static int parseMagFilter(final Attributes pAttributes) {
		final String magFilter = SAXUtils.getAttributeOrThrow(pAttributes, org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_MAGFILTER);
		if (magFilter.equals(org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_MAGFILTER_VALUE_NEAREST)) {
			return GL10.GL_NEAREST;
		} else if (magFilter.equals(org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_MAGFILTER_VALUE_LINEAR)) {
			return GL10.GL_LINEAR;
		} else {
			throw new IllegalArgumentException("Unexpected " + org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_MAGFILTER + " attribute: '" + magFilter + "'.");
		}
	}

	private int parseWrapT(final Attributes pAttributes) {
		return this.parseWrap(pAttributes, org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_WRAPT);
	}

	private int parseWrapS(final Attributes pAttributes) {
		return this.parseWrap(pAttributes, org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_WRAPS);
	}
	...
	private int parseWrap(final Attributes pAttributes, final String pWrapAttributeName) {
		...
		if (this.mVersion == 1 && wrapAttribute.equals(org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_WRAP_VALUE_CLAMP)) {
			return GL10.GL_CLAMP_TO_EDGE;
		} else if (wrapAttribute.equals(org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_WRAP_VALUE_CLAMP_TO_EDGE)) {
			return GL10.GL_CLAMP_TO_EDGE;
		} else if (wrapAttribute.equals(org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_WRAP_VALUE_REPEAT)) {
	...
	private boolean parsePremultiplyalpha(final Attributes pAttributes) {
		return SAXUtils.getBooleanAttributeOrThrow(pAttributes, org.andengine.util.texturepack.TexturePackParser.TAG_TEXTURE_ATTRIBUTE_PREMULTIPLYALPHA);
	}
