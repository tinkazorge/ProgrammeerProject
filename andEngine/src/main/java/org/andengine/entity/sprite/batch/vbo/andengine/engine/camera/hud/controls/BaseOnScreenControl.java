package org.andengine.entity.sprite.batch.vbo.andengine.engine.camera.hud.controls;
...
public abstract class BaseOnScreenControl extends HUD implements IOnSceneTouchListener {
	...
	private int mActivePointerID = org.andengine.engine.camera.hud.controls.BaseOnScreenControl.INVALID_POINTER_ID;
	...
	public BaseOnScreenControl(final float pX, final float pY, final Camera pCamera, final ITextureRegion pControlBaseTextureRegion, final ITextureRegion pControlKnobTextureRegion, final float pTimeBetweenUpdates, final VertexBufferObjectManager pVertexBufferObjectManager, final IOnScreenControlListener pOnScreenControlListener) {
		...
		this.mControlBase = new Sprite(pX, pY, pControlBaseTextureRegion, pVertexBufferObjectManager) {
			...
			public boolean onAreaTouched(final TouchEvent pSceneTouchEvent, final float pTouchAreaLocalX, final float pTouchAreaLocalY) {
				return org.andengine.engine.camera.hud.controls.BaseOnScreenControl.this.onHandleControlBaseTouched(pSceneTouchEvent, pTouchAreaLocalX, pTouchAreaLocalY);
			}
		};
		...
		this.registerUpdateHandler(new TimerHandler(pTimeBetweenUpdates, true, new ITimerCallback() {
			...
			public void onTimePassed(final TimerHandler pTimerHandler) {
				org.andengine.engine.camera.hud.controls.BaseOnScreenControl.this.mOnScreenControlListener.onControlChange(org.andengine.engine.camera.hud.controls.BaseOnScreenControl.this, org.andengine.engine.camera.hud.controls.BaseOnScreenControl.this.mControlValueX, org.andengine.engine.camera.hud.controls.BaseOnScreenControl.this.mControlValueY);
			}
		}));
		...
		if (pointerID == this.mActivePointerID) {
			...
			switch (pSceneTouchEvent.getAction()) {
				...
				case MotionEvent.ACTION_CANCEL:
					this.mActivePointerID = org.andengine.engine.camera.hud.controls.BaseOnScreenControl.INVALID_POINTER_ID;
			}
		}
		return false;
	}
	...
	protected boolean onHandleControlBaseTouched(final TouchEvent pSceneTouchEvent, final float pTouchAreaLocalX, final float pTouchAreaLocalY) {
		...
		switch (pSceneTouchEvent.getAction()) {
			case MotionEvent.ACTION_DOWN:
				if (this.mActivePointerID == org.andengine.engine.camera.hud.controls.BaseOnScreenControl.INVALID_POINTER_ID) {
	...
	public static interface IOnScreenControlListener {
		 ...
		 */
		public void onControlChange(final org.andengine.engine.camera.hud.controls.BaseOnScreenControl pBaseOnScreenControl, final float pValueX, final float pValueY);
	}
}
