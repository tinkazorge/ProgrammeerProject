...
public final class BezierCurveUtils {
	...
	public static final float getBezierCurveCoordinate(final float t, final float[] pControlPointCoordinates) {
		...
		for (int i = 0; i <= n; i++) {
			result += pControlPointCoordinates[i] * org.andengine.util.BezierCurveUtils.getBernsteinPolynomial(t, i, n);
		}
		return result;
	}
	...
	public static final float[] getBezierCurveCoordinates(final float t, final float[] pXs, final float[] pYs) {
		return org.andengine.util.BezierCurveUtils.getBezierCurveCoordinates(t, pXs, pYs, COORDINATES_TMP);
	}
	...
	public static final float[] getBezierCurveCoordinates(final float t, final float[] pXs, final float[] pYs, final float[] pReuse) {
		...
		for (int i = 0; i <= n; i++) {
			final float bernsteinPolynomial = org.andengine.util.BezierCurveUtils.getBernsteinPolynomial(t, i, n);
	...
	public static final float getBezierCurveLength(final float[] pXs, final float[] pYs) {
		return org.andengine.util.BezierCurveUtils.getBezierCurveLength(pXs, pYs, LENGTH_SAMPLES_DEFAULT);
	}
	...
	public static final float getBezierCurveLength(final float[] pXs, final float[] pYs, final int pSamples) {
		...
		for (int k = 1; k <= LENGTH_SAMPLES_DEFAULT; k++) {
			...
			float y = 0;
			for (int i = 0; i <= n; i++) {
				final float bernstein = org.andengine.util.BezierCurveUtils.getBernsteinPolynomial(t, i, n);
				x += pXs[i] * bernstein;
				y += pYs[i] * bernstein;
			}
	...
	public static final float getBezierCurveCurvature(final float t, final float[] pXs, final float[] pYs) {
		final float dxt = org.andengine.util.BezierCurveUtils.getBezierCurveDerivativeValue(t, 1, pXs);
		final float dyt = org.andengine.util.BezierCurveUtils.getBezierCurveDerivativeValue(t, 1, pYs);
		final float ddxt = org.andengine.util.BezierCurveUtils.getBezierCurveDerivativeValue(t, 2, pXs);
		final float ddyt = org.andengine.util.BezierCurveUtils.getBezierCurveDerivativeValue(t, 2, pYs);

		return org.andengine.util.BezierCurveUtils.getCurvature(dxt, dyt, ddxt, ddyt);
	}
	...
	public static final float getBezierCurveDerivativeValue(final float t, final int k, final float[] pControlPointCoordinates) {
		...
		float sum = 0;
		for (int i = 0; i <= n - k; i++) {
			final float bernstein = org.andengine.util.BezierCurveUtils.getBernsteinPolynomial(t, i, n - k);
			final float d = org.andengine.util.BezierCurveUtils.getLevelDifference(i, k, pControlPointCoordinates);
			sum += bernstein * d;
		}
	...
	private static float getLevelDifference(final int i, final int level, final float[] pControlPointCoordinates) {
		if (level == 0) {
			...
			return org.andengine.util.BezierCurveUtils.getLevelDifference(i + 1, level - 1, pControlPointCoordinates) - org.andengine.util.BezierCurveUtils.getLevelDifference(i, level - 1, pControlPointCoordinates);
		}
	}
