...
public class ParticleSystem<T extends IEntity> extends Entity {
	...
	protected final org.andengine.entity.particle.Particle<T>[] mParticles;
	...
	public ParticleSystem(final float pX, final float pY, final IEntityFactory<T> pEntityFactory, final IParticleEmitter pParticleEmitter, final float pRateMinimum, final float pRateMaximum, final int pParticlesMaximum) {
		...
		this.mParticles = (org.andengine.entity.particle.Particle<T>[]) new org.andengine.entity.particle.Particle[pParticlesMaximum];
	...
	protected void onManagedUpdate(final float pSecondsElapsed) {
		...
		for (int i = this.mParticlesAlive - 1; i >= 0; i--) {
			final org.andengine.entity.particle.Particle<T> particle = this.mParticles[i];
	...
	protected void moveParticleToEnd(final int pIndex) {
		final org.andengine.entity.particle.Particle<T> particle = this.mParticles[pIndex];
...
//		particles[this.mParticlesAlive] = particle;
	...
	private void spawnParticle() {
		if (this.mParticlesAlive < this.mParticlesMaximum) {
			org.andengine.entity.particle.Particle<T> particle = this.mParticles[this.mParticlesAlive];

			/* New particle needs to be created. */
			this.mParticleEmitter.getPositionOffset(org.andengine.entity.particle.ParticleSystem.POSITION_OFFSET_CONTAINER);

			final float x = org.andengine.entity.particle.ParticleSystem.POSITION_OFFSET_CONTAINER[Constants.VERTEX_INDEX_X];
			final float y = org.andengine.entity.particle.ParticleSystem.POSITION_OFFSET_CONTAINER[Constants.VERTEX_INDEX_Y];
