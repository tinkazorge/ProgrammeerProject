package org.andengine.entity.sprite.batch.vbo.andengine.util.uuid;
...
public class UUIDManager {
	...
	public static synchronized UUID getUUID(final Context pContext) {
		...
		return org.andengine.util.uuid.UUIDManager.getUUID(preferences);
	}

	public static UUID getUUID(final SharedPreferences pSharedPreferences) {
		final String uuidPreference = pSharedPreferences.getString(org.andengine.util.uuid.UUIDManager.PREFERENCES_UUIDMANAGER_UUID_KEY, null);
		...
		if (uuidPreference != null) {
			...
			editor.putString(org.andengine.util.uuid.UUIDManager.PREFERENCES_UUIDMANAGER_UUID_KEY, uuid.toString());
			editor.commit();
