...
public class Particle<T extends IEntity> {
	...
	private float mExpireTime = org.andengine.entity.particle.Particle.EXPIRETIME_NEVER;
	boolean mExpired;
	...
	protected void onUpdate(final float pSecondsElapsed) {
		if (!this.mExpired) {
			if (this.mExpireTime == org.andengine.entity.particle.Particle.EXPIRETIME_NEVER || this.mLifeTime + pSecondsElapsed < this.mExpireTime) {
	...
	public void reset() {
		...
		this.mExpired = false;
		this.mExpireTime = org.andengine.entity.particle.Particle.EXPIRETIME_NEVER;
		this.mLifeTime = 0;
	}
