package org.andengine.entity.sprite.batch.vbo.andengine.util.adt.bit;
...
import org.andengine.util.adt.bit.*;
 ...
 * @since Nov 20, 2012
 */
public class ByteBackedBitVector extends org.andengine.util.adt.bit.BitVector {
	// ===========================================================
	// Constants
	// ===========================================================
	...
	public ByteBackedBitVector(final int pSize) throws IllegalArgumentException {
		this(pSize, new byte[org.andengine.util.adt.bit.BitVector.calculateByteSize(pSize)]);
	}
	...
	public ByteBackedBitVector(final int pSize, final byte[] pData) throws IllegalArgumentException, NullPointerException {
		...
		if (org.andengine.util.adt.bit.BitVector.calculateByteSize(pSize) < pData.length) {
	...
	public ByteBackedBitVector(final DataInputStream pDataInputStream) throws IOException {
		...
		this.mData = new byte[org.andengine.util.adt.bit.BitVector.calculateByteSize(this.mSize)];

		pDataInputStream.readFully(this.mData);
	}

	public static org.andengine.util.adt.bit.ByteBackedBitVector load(final DataInputStream pDataInputStream) throws IOException {
		return new org.andengine.util.adt.bit.ByteBackedBitVector(pDataInputStream);
	}
	...
	public int getBit(final int pIndex) throws IllegalArgumentException {
		...
		final int byteIndex = org.andengine.util.adt.bit.BitVector.getByteIndex(pIndex);
		final int indexInByte = org.andengine.util.adt.bit.BitVector.getIndexInByte(pIndex);

		return org.andengine.util.adt.bit.BitVector.getBitInByte(this.mData[byteIndex], indexInByte);
	}

	@Override
	public boolean getBitAsBoolean(final int pIndex) throws IllegalArgumentException {
		return this.getBit(pIndex) == org.andengine.util.adt.bit.BitVector.TRUE;
	}
	...
	public void setBit(final int pIndex, final boolean pTrue) throws IllegalArgumentException {
		...
		final byte newByte = org.andengine.util.adt.bit.BitVector.setBitInByte(oldByte, indexInByte, pTrue);
	...
	public int getBits(final int pIndex, final int pCount) throws IllegalArgumentException {
		...
		for (int i = 0; i < bitsLeft; i++) {
			bits = bits << 1;
			if(this.getBit(index) == org.andengine.util.adt.bit.BitVector.TRUE) {
				bits |= 0x01;
			}
			index++;
		}
	...
	public void setBits(final int pIndex, final byte pBits, final int pBitIndex, final int pBitCount) throws IllegalArgumentException {
		...
		if (indexInByte == 0) {
			...
			final byte newByte = org.andengine.util.adt.bit.BitVector.setBitsInByte(this.mData[byteIndex], 0, pBits, pBitIndex, pBitCount);
			this.mData[byteIndex] = newByte;
		} else {
			final byte highByte = this.mData[byteIndex];
			final int highByteBitCount = Math.min(pBitCount, Byte.SIZE - indexInByte);

			final byte newHighByte = org.andengine.util.adt.bit.BitVector.setBitsInByte(highByte, indexInByte, pBits, pBitIndex, highByteBitCount);
			...
			if (highByteBitCount < pBitCount) {
				...
				final byte newLowByte = org.andengine.util.adt.bit.BitVector.setBitsInByte(lowByte, 0, pBits, lowByteBitIndex, lowByteBitCount);
	...
	public byte getByte(final int pIndex) throws IllegalArgumentException {
		...
		if (indexInByte == 0) {
			...
			final int highBits = org.andengine.util.adt.bit.BitVector.getBitsInByte(highByte, indexInByte, Byte.SIZE - indexInByte);
			final int lowBits = org.andengine.util.adt.bit.BitVector.getBitsInByte(lowByte, 0, indexInByte);
	...
	public final void setByte(final int pIndex, final byte pByte) throws IllegalArgumentException {
		...
		if (indexInByte == 0) {
			...
			this.mData[byteIndex] = org.andengine.util.adt.bit.BitVector.setBitsInByte(highByte, indexInByte, pByte, 0, Byte.SIZE - indexInByte);
			this.mData[byteIndex + 1] = org.andengine.util.adt.bit.BitVector.setBitsInByte(lowByte, 0, pByte, Byte.SIZE - indexInByte, indexInByte);
		}
	}
