package org.andengine.entity.sprite.batch.vbo.andengine.engine.camera;
...
public class Camera implements IUpdateHandler {
	...
	public float[] getCameraSceneCoordinatesFromSceneCoordinates(final float pSceneX, final float pSceneY) {
		org.andengine.engine.camera.Camera.VERTICES_TMP[Constants.VERTEX_INDEX_X] = pSceneX;
		org.andengine.engine.camera.Camera.VERTICES_TMP[Constants.VERTEX_INDEX_Y] = pSceneY;

		return this.getCameraSceneCoordinatesFromSceneCoordinates(org.andengine.engine.camera.Camera.VERTICES_TMP);
	}
	...
	public float[] getSceneCoordinatesFromCameraSceneCoordinates(final float pCameraSceneX, final float pCameraSceneY) {
		org.andengine.engine.camera.Camera.VERTICES_TMP[Constants.VERTEX_INDEX_X] = pCameraSceneX;
		org.andengine.engine.camera.Camera.VERTICES_TMP[Constants.VERTEX_INDEX_Y] = pCameraSceneY;

		return this.getSceneCoordinatesFromCameraSceneCoordinates(org.andengine.engine.camera.Camera.VERTICES_TMP);
	}
	...
	private void applySceneRotation(final TouchEvent pCameraSceneTouchEvent) {
		...
		if (rotation != 0) {
			org.andengine.engine.camera.Camera.VERTICES_TMP[Constants.VERTEX_INDEX_X] = pCameraSceneTouchEvent.getX();
			org.andengine.engine.camera.Camera.VERTICES_TMP[Constants.VERTEX_INDEX_Y] = pCameraSceneTouchEvent.getY();

			MathUtils.rotateAroundCenter(org.andengine.engine.camera.Camera.VERTICES_TMP, rotation, this.getCenterX(), this.getCenterY()); // TODO Use a Transformation object instead!?!

			pCameraSceneTouchEvent.set(org.andengine.engine.camera.Camera.VERTICES_TMP[Constants.VERTEX_INDEX_X], org.andengine.engine.camera.Camera.VERTICES_TMP[Constants.VERTEX_INDEX_Y]);
		}
	}
	...
	private void unapplySceneRotation(final TouchEvent pSceneTouchEvent) {
		...
		if (rotation != 0) {
			org.andengine.engine.camera.Camera.VERTICES_TMP[Constants.VERTEX_INDEX_X] = pSceneTouchEvent.getX();
			org.andengine.engine.camera.Camera.VERTICES_TMP[Constants.VERTEX_INDEX_Y] = pSceneTouchEvent.getY();

			MathUtils.revertRotateAroundCenter(org.andengine.engine.camera.Camera.VERTICES_TMP, rotation, this.getCenterX(), this.getCenterY()); // TODO Use a Transformation object instead!?!

			pSceneTouchEvent.set(org.andengine.engine.camera.Camera.VERTICES_TMP[Constants.VERTEX_INDEX_X], org.andengine.engine.camera.Camera.VERTICES_TMP[Constants.VERTEX_INDEX_Y]);
		}
	}
	...
	private void applyCameraSceneRotation(final TouchEvent pSceneTouchEvent) {
		...
		if (cameraSceneRotation != 0) {
			...
			MathUtils.rotateAroundCenter(org.andengine.engine.camera.Camera.VERTICES_TMP, cameraSceneRotation, this.getCameraSceneWidth() * 0.5f, this.getCameraSceneHeight() * 0.5f); // TODO Use a Transformation object instead!?!
	...
	private void unapplyCameraSceneRotation(final TouchEvent pCameraSceneTouchEvent) {
		...
		if (cameraSceneRotation != 0) {
			...
			MathUtils.revertRotateAroundCenter(org.andengine.engine.camera.Camera.VERTICES_TMP, cameraSceneRotation, this.getCameraSceneWidth() * 0.5f, this.getCameraSceneHeight() * 0.5f); // TODO Use a Transformation object instead!?!
	...
	public void convertSurfaceTouchEventToSceneTouchEvent(final TouchEvent pSurfaceTouchEvent, final int pSurfaceWidth, final int pSurfaceHeight) {
		...
		if (rotation == 0) {
			...
			org.andengine.engine.camera.Camera.VERTICES_TMP[Constants.VERTEX_INDEX_X] = surfaceTouchEventX;
			org.andengine.engine.camera.Camera.VERTICES_TMP[Constants.VERTEX_INDEX_Y] = surfaceTouchEventY;

			MathUtils.rotateAroundCenter(org.andengine.engine.camera.Camera.VERTICES_TMP, rotation, pSurfaceWidth >> 1, pSurfaceHeight >> 1); // TODO Use a Transformation object instead!?!

			relativeX = org.andengine.engine.camera.Camera.VERTICES_TMP[Constants.VERTEX_INDEX_X] / pSurfaceWidth;
			relativeY = 1 - (org.andengine.engine.camera.Camera.VERTICES_TMP[Constants.VERTEX_INDEX_Y] / pSurfaceHeight);
		}
	...
	public float[] getSceneCoordinatesFromSurfaceCoordinates(final float pSurfaceX, final float pSurfaceY) {
		org.andengine.engine.camera.Camera.VERTICES_TMP[Constants.VERTEX_INDEX_X] = pSurfaceX;
		org.andengine.engine.camera.Camera.VERTICES_TMP[Constants.VERTEX_INDEX_Y] = pSurfaceY;

		return this.getSceneCoordinatesFromSurfaceCoordinates(org.andengine.engine.camera.Camera.VERTICES_TMP);
	}
	...
	public void convertSceneTouchEventToSurfaceTouchEvent(final TouchEvent pSceneTouchEvent, final int pSurfaceWidth, final int pSurfaceHeight) {
		...
		if (rotation == 0) {
			...
			MathUtils.revertRotateAroundCenter(org.andengine.engine.camera.Camera.VERTICES_TMP, -rotation, pSurfaceWidth >> 1, pSurfaceHeight >> 1); // TODO Use a Transformation object instead!?!
	...
	public float[] getSurfaceCoordinatesFromSceneCoordinates(final float pSceneX, final float pSceneY, final int pSurfaceWidth, final int pSurfaceHeight) {
		...
		return this.getSurfaceCoordinatesFromSceneCoordinates(org.andengine.engine.camera.Camera.VERTICES_TMP, pSurfaceWidth, pSurfaceHeight);
	}
	...
	private void allocateUpdateHandlers() {
		this.mUpdateHandlers = new UpdateHandlerList(org.andengine.engine.camera.Camera.UPDATEHANDLERS_CAPACITY_DEFAULT);
	}
